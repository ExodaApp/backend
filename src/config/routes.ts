...
routes.post(
  ...
  validateRequest({
    body: z.object({
      userAddress: z
        .string()
        .length(42, "Address is not a valid ethereum userAddress"),
    }),
  }),
  userController.createUser
);
routes.get("/user/:userAddress", isAuthenticated, userController.getUser);
...
routes.get(
  "/exchange-wallet",
  isAuthenticated,
  exchangeWalletController.getExchangeWallets
);
routes.post(
  "/exchange-wallet",
  [
  ...
  [
    validateRequest({
      params: z.object({
        tokenAddress: z
          .string()
          .length(42, "Token addresss is not a valid ethereum address"),
      }),
      query: z.object({
