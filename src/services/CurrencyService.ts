...
type CurrenciesToUsdExchangeRate = Omit<
  Record<Currency, BigNumber>,
  Currency.USD
>;
...
export class CurrencyService {
  ...
  public static async getUsdExchangeRate(
    currency: Currency
  ): Promise<BigNumber> {
    if (currency === Currency.USD) return new BigNumber("1");
  ...
  private static async _fetchAwesomeApiRates(): Promise<CurrenciesToUsdExchangeRate> {
    const usdPairs = Object.keys(Currency).filter(
      (key) => key !== Currency.USD
    );
    ...
    console.log({
      API_CALL: `${CurrencyService._awesomeApiUrl}/${requestParams}`,
    });

    const { data } = await axios(
      `${CurrencyService._awesomeApiUrl}/${requestParams}`
    );
